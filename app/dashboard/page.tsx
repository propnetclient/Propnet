...
export default function Dashboard() {
  const { data: myProperties = [], isLoading: propertiesLoading } = useQuery<any[]>({
    queryKey: ["/api/my-properties"],
  });

  const { data: myRequirements = [], isLoading: requirementsLoading } = useQuery<any[]>({
    queryKey: ["/api/my-requirements"],
  });

  const { data: allProperties = [], isLoading: allPropertiesLoading } = useQuery<any[]>({
    queryKey: ["/api/properties"],
  });

  const { data: coListingRequests = [], isLoading: coListingLoading } = useQuery<any[]>({
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="bg-white border-b border-neutral-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-neutral-900">Welcome to Dashboard</h1>
            <p className="text-sm text-neutral-600">Manage your properties and listings</p>
          </div>
          <Link href="/profile">
