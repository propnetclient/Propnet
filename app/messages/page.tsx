      ...
      <div className="min-h-screen bg-neutral-50 flex flex-col">
        ...
        <div className="flex-1 px-6 py-4 space-y-4 overflow-y-auto">
          {Array.isArray(messages) && messages.map((message: any) => (
            <div
              key={message.id}
              className={`flex ${message.senderId === (null as any)?.id ? 'justify-end' : 'justify-start'}`}
            >
              <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${
                message.senderId === (null as any)?.id 
                  ? 'bg-primary text-white' 
                  : 'bg-white border border-neutral-200'
              }`}>
                <p className="text-sm">{message.content}</p>
                <p className={`text-xs mt-1 ${
                  message.senderId === (null as any)?.id ? 'text-primary-200' : 'text-neutral-500'
                }`}>
